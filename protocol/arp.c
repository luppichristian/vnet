...
bool arp_write_ethernet_reply(FILE* destination, const arp_reply_data_t* packet_data) {
  ...
  memcpy(packet.sender_hardware_address, packet_data->sender_hardware_address, sizeof(packet.sender_hardware_address));
  memcpy(packet.target_hardware_address, packet_data->target_hardware_address, sizeof(packet.target_hardware_address));
  ...
  memcpy(frame.dst_addr, packet_data->target_hardware_address, sizeof(frame.dst_addr));
  memcpy(frame.src_addr, packet_data->sender_hardware_address, sizeof(frame.src_addr));
  return ethernet_write_frame(destination, &frame);
}
