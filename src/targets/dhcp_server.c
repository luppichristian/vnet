...
static void command_info(void* argument, char* arguments) {
  ...
  fputs("DHCP server IPv4: ", stdout); ipv4_address_print(stdout, context->address);
  fputs("\nLease client: ", stdout); ethernet_mac_print(stdout, context->client_mac);
  fputs("\nLease address: ", stdout); ipv4_address_print(stdout, context->client_address);
  fputs("\n", stdout);
}
...
static void handle_frame(dhcp_server_context_t* context, const uint8_t* bytes, size_t count) {
  ethernet_frame_view_t frame = {0}; ipv4_packet_view_t ipv4 = {0}; udp_packet_view_t udp = {0}; dhcp_message_t message = {0};
  ...
  else if (message.type == DHCP_MESSAGE_REQUEST && message.client_address == context->client_address && message.server_address == context->address && reply(context, &frame, &message, DHCP_MESSAGE_ACK)) fputs("DHCP ACK sent.\n", stdout);
}

int main(int argc, char** argv) {
  if (argc != 9) { fputs("Usage: dhcp_server <file> <mac-address> <server-ip> <client-mac> <client-ip> <mask> <gateway> <dns-server>\n", stderr); return EXIT_FAILURE; }
  ...
  context.source = fopen(context.path, "rb"); if (!context.source || fseek(context.source, 0, SEEK_END) != 0) return EXIT_FAILURE;
  cmd_app_init(&context.commands); if (!cmd_app_register(&context.commands, "info", "Show the configured client lease.", command_info, &context) || !cmd_app_start(&context.commands)) return EXIT_FAILURE;
  uint8_t buffer[DHCP_SERVER_BUFFER_SIZE];
  while (cmd_app_is_running(&context.commands)) { long end = 0; if (!get_file_end(context.source, &end)) break; long position = ftell(context.source); if (position < end) { size_t count = (size_t)(end - position) < sizeof(buffer) ? (size_t)(end - position) : sizeof(buffer); if (fread(buffer, 1, count, context.source) != count) break; for (size_t start = 0, next = 0; start < count; start = next) { for (next = start + 1; next < count && !ethernet_frame_is_start(buffer + next, count - next); ++next) {} handle_frame(&context, buffer + start, next - start); } } thread_sleep(5); }
  cmd_app_stop(&context.commands); cmd_app_join(&context.commands); fclose(context.source); return EXIT_SUCCESS;
}
