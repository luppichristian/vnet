...
static bool print_ethernet_frame(const uint8_t* bytes, size_t byte_count) {
  ...
  if (header.sfd != ETHERNET_SFD || header.length > ETHERNET_MAX_DATA_LEN) {
    return false;
  }

  const uint16_t padded_data_length = header.length < ETHERNET_MIN_DATA_LEN ? ETHERNET_MIN_DATA_LEN : header.length;
  const size_t expected_frame_length = sizeof(header) + padded_data_length + sizeof(ethernet_footer_t);
  ...
  const size_t crc_data_length = sizeof(header.dst_mac) + sizeof(header.src_mac) + sizeof(header.length) + padded_data_length;
  if (crc32(crc_data, crc_data_length) != footer.crc) {
    return false;
  }

  fprintf(stdout, "Valid IEEE 802.3 Ethernet frame (%zu bytes):\n", expected_frame_length);
  ...
  fprintf(stdout, "  Padding:         %u bytes\n", padded_data_length - header.length);
